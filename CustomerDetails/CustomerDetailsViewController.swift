...
//  EHS_Sales
//
//  Created by Anju on 27.03.2023.
//

import UIKit

class CustomerDetailsViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        if customer==0{
            setNavigationBarbacklogoName(name: "Edit Details")
        }else{
            setNavigationBarbacklogoName(name: "Edit Details")
        }
        ...
        customerDetailsTableView.register(UINib(nibName: "CustomerDetail2TableViewCell", bundle: nil), forCellReuseIdentifier: "CustomerDetail2TableViewCell")
...
//        customerOneDetailmodel.homePhone=customerlist.homePhoneNumber?.toPhoneNumber() ?? ""
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        //        if customer==0{
        customerOneCell = tableView.dequeueReusableCell(withIdentifier: "CustomerDetail2TableViewCell") as!  CustomerDetail2TableViewCell
        customerOneCell.homephoneTxtFld.delegate=self
        customerOneCell.contactNumberTxtfld.delegate=self
        ...
        customerOneCell.coapplicantFirstName.text=nofinanceModel.coapplicantfirstName
        customerOneCell.coapplicantLastName.text=nofinanceModel.coapplicantlastName
        customerOneCell.coapplicantMiddleName.text=nofinanceModel.coapplicantmiddleName
        ...
        customerOneCell.coapplicantFirstName.delegate=self
        customerOneCell.coapplicantLastName.delegate=self
        customerOneCell.coapplicantMiddleName.delegate=self
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if adressType=="locationAddress"{
            return 1000
        }else{
            return 780
        }
    }
    //    @objc func addCustomerAction(sender:UIButton){
    //        if cell.addcustmeRImageView.image==UIImage(named:"addCustomerUntick"){
    //            cell.addcustmeRImageView.image=UIImage(named:"addCustomerTick")
    //            addCustomerDetails=1
    //
    //        }else{
    //            cell.addcustmeRImageView.image=UIImage(named:"addCustomerUntick")
    //            addCustomerDetails=0
    //
    //        }
    ...
    @objc func updateAction(){
        if customerOneDetailmodel.firstName.trimmingCharacters(in: .whitespaces).isEmpty{
            ...
            self.showAlertOk(message: "tate is required. Please enter to proceed.")
        }else{
            var coapplicantFullName=""
            if nofinanceModel.coapplicantfirstName != ""{
                let firstname=nofinanceModel.coapplicantfirstName
                coapplicantFullName = firstname
                if nofinanceModel.coapplicantmiddleName != ""{
                    let middlename=nofinanceModel.coapplicantmiddleName
                   coapplicantFullName = [coapplicantFullName,middlename].joined(separator: " ")
                }
                if nofinanceModel.coapplicantlastName != ""{
                    let lastname=nofinanceModel.coapplicantlastName
                    coapplicantFullName = [coapplicantFullName,lastname].joined(separator: " ")
                }
            }
            

            if !coapplicantFullName.isEmpty {
               let seconsigner = coapplicantFullName
                UserDefaults.standard.set(seconsigner, forKey: "secondsignor")
            } else {
                // Handle the case where all coapplicant names are empty
                print("Coapplicant names are all empty.") // Or perform alternative actions
            }
            updateAdress()
        }
    ...
    func update(){
        ...
        let customerid=customerlist.customerId
        ...
        parameters = ["customerId":customerid ?? 0,"appointmentId":appointmentId ?? 0,"firstname":firstName ?? "","lastName":lastName ?? "","middleName":middleName ?? "","cellPhoneNumber":cellPhoneNumber ?? "","homePhoneNumber":homePhoneNumber ?? "","emailAddress":emailAddress ?? "","addressLineOne":addressLineOne ?? "","city":city ?? "","state":state ?? "","zip":zip ?? "","gateCode":gateCode ?? "","crossStreets":crossStreets ?? ""]
        print("parameters",parameters)
        customerupdateViewModel.customer_update(rawdata: parameters) { success, CustomerList, message in
            if success{
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if textField==customerOneCell.coapplicantFirstName{
            let text = (textField.text! as NSString).replacingCharacters(in: range, with: string)
            nofinanceModel.coapplicantfirstName=text
            }
            if textField==customerOneCell.coapplicantLastName{
                let text = (textField.text! as NSString).replacingCharacters(in: range, with: string)
                nofinanceModel.coapplicantlastName=text
            }
            if textField==customerOneCell.coapplicantMiddleName{
                let text = (textField.text! as NSString).replacingCharacters(in: range, with: string)
                nofinanceModel.coapplicantmiddleName=text
            }
...
//    }
    ...
    private func customerDetails(data: CustomerList) {
        ...
        customerlist=data
        modelUpdated()
        let customerEdit = CustomerDetailsEditViewController.initialization()!
